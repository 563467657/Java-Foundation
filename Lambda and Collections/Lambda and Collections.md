...
# Java 8 Lambda and Collections

[TOC]

## 前言

入职这段时间，基本上公司的进销存项目代码和confluence上的文章都过了一遍，对于整体项目的代码和技术，也有一些自己的心得体会，也在这里和大家分享一下。

java8已经发布很久了，对于开发人员，个人觉得最能提升编程效率是lambda表达式和函数式接口，从结果来看，lambda正在被越来越多的开发人员接受，我们项目中也有大量的使用lambda，从进步的角度来讲，我们也应该积极拥抱lambda，confluence文档中最新分享也提到了java8函数式接口的一些概念，包括由`@FunctionalInterface`所显式标注的支持函数式编程的接口 和jdk所提供的函数式接口，而我们今天，从lambda的角度来看看java8在集合框架中都做了哪些提升。
...
![](https://raw.githubusercontent.com/563467657/Java-Foundation/master/Lambda%20and%20Collections/JCF_Collection_Interfaces.jpg)
...
这些新加入的方法大部分要用到`java.util.function`包下的接口，这意味着这些方法大部分都跟Lambda表达式相关。我将逐一介绍这些方法。

## Collection中的新方法

如上所示，接口`Collection`和`List`新加入了一些方法，我们以是`List`的子类`ArrayList`为例来说明。
...
`stream()`和`parallelStream()`分别**返回该容器的`Stream`视图表示**，不同之处在于`parallelStream()`返回并行的`Stream`。**`Stream`是Java函数式编程的核心类**，因为内容较多，我们会在以后的文档中分享。
...
1. Java8为容器新增了一些新的方法，这些方法有些是为**完善原有功能**，有些是为**引入函数式编程**，学习和使用这些方法有助于我们写出更加简洁有效的代码．
